	...
	public class ThingSettingsView : Container
	{
		...
		protected void BuildThingSettingsPanel()
		{
			...
			foreach (var thingItemAndIndex in CurrentThingSettings.Things.Select((Thing value, int i) => new { i, value }))
			{
				...
				val3.set_TextColor(isAvailable ? Color.get_White() : Color.get_Red());
				((Control)val3).set_BasicTooltipText(isAvailable ? null : "No keybind is set in the General Settings tab");
				val3.set_Text($"{index + 1}. {thing.Name}");
				Label thingInSettings_Label = val3;
				StandardButton val4 = new StandardButton();
				((Control)val4).set_Parent((Container)(object)panel);
				((Control)val4).set_Location(new Point(((Control)thingInSettings_Label).get_Right(), ((Control)thingInSettings_Label).get_Top()));
