...
namespace Manlaan.CommanderMarkers.CornerIcon
{
	public class CornerIconToggleMenuItem : ContextMenuStripItem
	{
		public CornerIconToggleMenuItem(SettingEntry<bool> setting, string displayLabel)
		{
			...
			((ContextMenuStripItem)this).set_Text((setting2.get_Value() ? "Hide" : "Show") + " " + baseText);
			...
			{
				((ContextMenuStripItem)cornerIconToggleMenuItem).set_Text((setting2.get_Value() ? "Hide" : "Show") + " " + baseText);
			});
		}
