...
namespace Blish_HUD.Extended
{
	public static class Gw2ApiManagerExtensions
	{
		public static bool IsApiAvailable(this Gw2ApiManager gw2ApiManager, bool showMessage = false)
		{
			...
			try
			{
				HttpResponseMessage response = "https://api.guildwars2.com/".AllowHttpStatus(HttpStatusCode.ServiceUnavailable).AllowHttpStatus(HttpStatusCode.InternalServerError).GetAsync(CancellationToken.None, (HttpCompletionOption)1)
					.Result;
				try
				{
					...
					else if (response.get_StatusCode() == HttpStatusCode.ServiceUnavailable)
					{
						...
						string paragraph = result.GetTextBetweenTags("p").Split(new char[1] { '.' }, StringSplitOptions.RemoveEmptyEntries).AsEnumerable()
							.Reverse()
							.First()
							.Trim();
