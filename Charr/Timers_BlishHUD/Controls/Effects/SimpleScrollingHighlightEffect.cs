...
namespace Charr.Timers_BlishHUD.Controls.Effects
{
	public class SimpleScrollingHighlightEffect : ControlEffect
	{
			...
			: base(assignedControl)
		{
			...
			_scrollEffect.get_Parameters().get_Item("Opacity").SetValue(assignedControl.Opacity);
		}

		protected override SpriteBatchParameters GetSpriteBatchParameters()
		{
			//IL_0018: Unknown result type (might be due to invalid IL or missing references)
			return new SpriteBatchParameters((SpriteSortMode)1, BlendState.AlphaBlend, SamplerState.LinearWrap, null, null, _scrollEffect, GameService.Graphics.UIScaleTransform);
		}
		...
		protected override void OnEnable()
		{
			...
			{
				_scrollEffect.get_Parameters().get_Item("Opacity").SetValue(base.AssignedControl.Opacity);
				ScrollRoller = 0f;
				_shaderAnim = GameService.Animation.Tweener.Tween(this, new
				{
					ScrollRoller = 1f
				}, _duration);
				_active = true;
		...
		public override void PaintEffect(SpriteBatch spriteBatch, Rectangle bounds)
		{
			...
			{
				spriteBatch.DrawOnCtrl(base.AssignedControl, ContentService.Textures.Pixel, bounds, Color.get_Transparent());
			}
		}
	}
}
