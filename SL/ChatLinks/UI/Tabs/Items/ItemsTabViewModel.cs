	...
	public sealed class ItemsTabViewModel : ViewModel, IDisposable
	{
		...
		public async Task OnSearch()
		{
			...
			try
			{
				...
				try
				{
					await Task.Delay(300, cancellationTokenSource.Token).ConfigureAwait(continueOnCapturedContext: false);
					await _searchLock.WaitAsync(cancellationTokenSource.Token).ConfigureAwait(continueOnCapturedContext: false);
					try
					{
						await DoSearch(cancellationTokenSource.Token).ConfigureAwait(continueOnCapturedContext: false);
					}
					finally
					{
						_searchLock.Release();
		...
		private async Task DoSearch(CancellationToken cancellationToken)
		{
			...
			if (length < 3)
			{
				if (length == 0)
				{
					await NewItems(cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
				}
			}
			else
			{
				await Query(query, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
		}
		...
		private async Task Query(string text, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				await foreach (Item item in _003Csearch_003EP.Search(text, maxResults, context, cancellationToken).ConfigureAwait(continueOnCapturedContext: false))
				{
