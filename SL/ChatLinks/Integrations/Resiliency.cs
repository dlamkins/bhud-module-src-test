...
namespace SL.ChatLinks.Integrations
{
	internal static class Resiliency
	{
		...
		private static async Task<string?> GetText(Outcome<HttpResponseMessage> attempt)
		{
			...
			await attempt.Result!.Content.LoadIntoBufferAsync().ConfigureAwait(continueOnCapturedContext: false);
			Stream content = await attempt.Result!.Content.ReadAsStreamAsync().ConfigureAwait(continueOnCapturedContext: false);
			try
			{
				using JsonDocument json = await JsonDocument.ParseAsync(content).ConfigureAwait(continueOnCapturedContext: false);
				JsonElement text;
