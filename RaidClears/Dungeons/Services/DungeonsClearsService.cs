...
namespace RaidClears.Dungeons.Services
{
	public static class DungeonsClearsService
	{
		...
		public static async Task<(ApiDungeons, bool apiAccessFailed)> GetDungeonClearsFromApi(Gw2ApiManager gw2ApiManager, Logger logger)
		{
		...
		private static async Task<ApiDungeons> GetCurrentClearsFromApi(Gw2ApiManager gw2ApiManager, Logger logger)
		{
			IApiV2ObjectList<string> weeklyCleared = await ((IBlobClient<IApiV2ObjectList<string>>)(object)gw2ApiManager.get_Gw2ApiClient().get_V2().get_Account()
				.get_Dungeons()).GetAsync(default(CancellationToken));
			...
			return new ApiDungeons(((IEnumerable<string>)weeklyCleared).ToList(), list);
		}

		public static List<string> ConvertFrequenterToPathId(List<int> frequentedPaths)
		{
			List<string> list = new List<string>();
			foreach (int path in frequentedPaths)
			{
