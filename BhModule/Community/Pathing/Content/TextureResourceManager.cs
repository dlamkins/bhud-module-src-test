	...
	public class TextureResourceManager : IPackResourceManager
	{
		...
		private static void LoadTexture(TaskCompletionSource<Texture2D> textureTcs, byte[] textureData)
		{
			GameService.Graphics.QueueMainThreadRender((Action<GraphicsDevice>)delegate(GraphicsDevice graphicsDevice)
			{
				try
				{
					textureTcs.SetResult(TextureUtil.FromStreamPremultiplied(graphicsDevice, (Stream)new MemoryStream(textureData)));
				}
				catch (Exception)
				{
		...
		public async Task PreloadTexture(string texturePath)
		{
			if (!_textureCache.ContainsKey(texturePath))
			{
				...
				LoadTexture(textureTcs2, await LoadResourceAsync(texturePath));
			}
		}
