	...
	public class InputDialog<TControl> : Container where TControl : Control, new()
	{
		...
		public object Input
		{
			get
			{
				...
				if (controlType.Name == typeof(Dropdown<>).Name)
				{
					Type genericTypeBase = controlType.GetGenericTypeDefinition();
					Type[] genericArgs = controlType.GetGenericArguments();
					Type genericType = genericTypeBase.MakeGenericType(genericArgs);
					object dropdown2 = Convert.ChangeType(_inputControl, genericType);
					return dropdown2.GetType().GetProperty("SelectedItem", BindingFlags.Instance | BindingFlags.Public).GetValue(dropdown2);
				}
				TControl inputControl = _inputControl;
				object obj = inputControl;
				TextBox textBox = (TextBox)((obj is TextBox) ? obj : null);
				if (textBox != null)
				{
					return ((TextInputBase)textBox).get_Text()?.Trim();
				}
				object obj2 = inputControl;
				Checkbox checkbox = (Checkbox)((obj2 is Checkbox) ? obj2 : null);
				if (checkbox != null)
				{
					return checkbox.get_Checked();
				}
				object obj3 = inputControl;
				Dropdown dropdown = (Dropdown)((obj3 is Dropdown) ? obj3 : null);
				if (dropdown != null)
				{
					return dropdown.get_SelectedItem();
				}
				throw new NotSupportedException("The control " + typeof(TControl).FullName + " is currently not supported.");
			}
		}
		...
		public InputDialog(TControl control, string title, string message, IconService iconService, ButtonDefinition[] buttons = null)
		{
			...
			((Control)this).set_ZIndex(715827882);
		...
		{
			((Control)_inputControl).set_Parent((Container)(object)this);
		}
		...
		public override void UpdateContainer(GameTime gameTime)
		{
			...
			((Control)_inputControl).set_Width((int)((double)_confirmRect.Width * 0.75));
			int inputControlY = _confirmRect.Y + _confirmRect.Height / 2 + _confirmRect.Height / 6;
			((Control)_inputControl).set_Location(new Point(_confirmRect.X + _confirmRect.Width / 2 - ((Control)_inputControl).get_Width() / 2, inputControlY));
