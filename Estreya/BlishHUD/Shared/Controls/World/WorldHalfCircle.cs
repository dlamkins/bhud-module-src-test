...
namespace Estreya.BlishHUD.Shared.Controls.World
{
	...
	{
		public WorldHalfCircle(Vector3 position, float radius, int tessellation = 50)
		...
		public WorldHalfCircle(Vector3 position, float radius, Color color, int tessellation = 50)
			: base(position, Enumerable.Range(0, tessellation + 1).SelectWithIndex((Func<int, int, IEnumerable<int>, Vector3>)delegate(int t, int index, IEnumerable<int> sourceList)
			{
				...
				float num = (float)((double)((float)index / (float)tessellation * 1f) * Math.PI);
