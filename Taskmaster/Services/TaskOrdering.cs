...
namespace Taskmaster.Services
{
	public static class TaskOrdering
	{
		...
		{
			return Ordered(tasks).IndexOf(task);
		}
		...
		public static bool MoveByVisible(IList<TodoTask> tasks, TodoTask task, int delta, Func<TodoTask, bool> isVisible)
		{
			if (tasks == null || task == null || isVisible == null)
			{
				return false;
			}
			List<TodoTask> ordered = Ordered(tasks);
			List<TodoTask> visible = ordered.Where((TodoTask candidate) => candidate == task || isVisible(candidate)).ToList();
			int num = visible.IndexOf(task);
			int targetVisibleIndex = num + delta;
			if (num < 0 || targetVisibleIndex < 0 || targetVisibleIndex >= visible.Count)
			{
				return false;
			}
			TodoTask target = visible[targetVisibleIndex];
		...
		{
			return MoveToIndex(tasks, task, 0);
		}

		public static bool MoveToEnd(IList<TodoTask> tasks, TodoTask task)
		{
			return MoveToIndex(tasks, task, Math.Max(0, tasks.Count - 1));
		}
		...
		{
			return (from item in tasks.Select((TodoTask task, int index) => new { task, index })
				orderby item.task.Order, item.index
				select item.task).ToList();
		}
