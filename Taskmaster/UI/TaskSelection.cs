...
namespace Taskmaster.UI
{
	public sealed class TaskSelection
	{
		...
		public bool IsSelected(Guid taskId)
		{
			return _selectedIds.Contains(taskId);
		}
		...
		public void Select(Guid taskId, IReadOnlyList<Guid> orderedTaskIds, bool extendRange, bool toggle)
		{
			...
			if (extendRange && _anchorId.HasValue)
			{
				...
				if (anchorIndex >= 0)
				{
					...
					{
						_selectedIds.Add(orderedTaskIds[i]);
					}
					return;
				}
			}
		...
		{
			_selectedIds.RemoveWhere((Guid id) => IndexOf(orderedTaskIds, id) < 0);
