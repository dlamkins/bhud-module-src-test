...
namespace Taskmaster.UI
{
	public sealed class TaskmasterSizing
	{
		...
		{
			InterfaceScale = Clamp(interfaceScale, 0.8f, 1.5f);
			TextScale = Clamp(textScale, 0.85f, 1.4f);
		}

		public int Px(int value)
		{
			return Math.Max(1, (int)Math.Round((float)value * InterfaceScale, MidpointRounding.AwayFromZero));
		}
		...
		private static BitmapFont ClosestFont(float targetSize)
		{
			...
			if (targetSize < 15f)
			{
				return GameService.Content.get_DefaultFont14();
			}
			if (targetSize < 17f)
			{
				return GameService.Content.get_DefaultFont16();
			}
			return GameService.Content.get_DefaultFont18();
		}
	}
}
