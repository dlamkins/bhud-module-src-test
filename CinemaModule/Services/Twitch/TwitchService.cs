	...
	public class TwitchService : IDisposable
	{
		...
		private async Task<List<TwitchStreamInfo>> EnrichWithUserAvatars(List<TwitchStreamInfo> streams)
		{
			...
			try
			{
				...
				JArray users = JObject.Parse(await response.get_Content().ReadAsStringAsync())["data"] as JArray;
				if (users == null)
				{
					return streams;
				}
				...
				foreach (JToken item in users)
				{
					...
					string avatar = item["profile_image_url"]?.ToString();
		...
		private List<TwitchStreamInfo> ParseHelixFollowedStreams(JObject json)
		{
			...
			{
				string login = stream["user_login"]?.ToString();
		...
		public async Task<List<TwitchStreamQuality>> GetStreamQualitiesAsync(string channelName)
		{
			...
			catch (HttpRequestException val)
			{
				...
				Logger.Warn("Stream unavailable for channel '" + channelName + "': " + ((Exception)(object)ex2).Message);
				return new List<TwitchStreamQuality>();
			}
			catch (Exception ex)
			{
		...
		public string SelectQuality(int qualityIndex)
		{
			...
			TwitchStreamQuality twitchStreamQuality = _cachedQualities[qualityIndex];
			List<string> qualityNames = _cachedQualities.Select((TwitchStreamQuality q) => q.DisplayName).ToList();
			this.QualitiesChanged?.Invoke(this, new TwitchQualitiesEventArgs(qualityNames, _selectedQualityIndex));
			return twitchStreamQuality.StreamUrl;
		}
		...
		private JObject BuildStreamInfoQuery(string channelName)
		{
			return new JObject
			{
				...
				["variables"] = new JObject { ["login"] = (JToken)channelName.ToLowerInvariant() }
			};
		}

		private JObject BuildMultipleStreamInfoQuery(List<string> channelNames)
		{
			JArray loginsArray = new JArray(channelNames);
			return new JObject
			{
		...
		private JObject BuildPlaybackAccessTokenQuery(string channelName)
		{
			return new JObject
			{
				["operationName"] = (JToken)"PlaybackAccessToken_Template",
				...
				{
					["isLive"] = (JToken)true,
					["login"] = (JToken)channelName.ToLowerInvariant(),
					["isVod"] = (JToken)false,
					["vodID"] = (JToken)"",
					["playerType"] = (JToken)"site"
				}
			};
		}
		...
		private async Task<JObject> ExecuteGqlRequestAsync(JObject query, string operationName, bool useAuth)
		{
			...
			val.set_Content((HttpContent)new StringContent(query.ToString(), Encoding.UTF8, "application/json"));
		...
		private TwitchStreamInfo ParseStreamInfo(JObject json, string channelName)
		{
			...
			return new TwitchStreamInfo
			{
				ChannelName = channelName,
				IsLive = isLive,
				Title = ((!isLive) ? null : stream["title"]?.ToString()),
				GameName = ((!isLive) ? null : stream["game"]?["name"]?.ToString()),
		...
		{
			Dictionary<string, TwitchStreamInfo> result = new Dictionary<string, TwitchStreamInfo>(StringComparer.OrdinalIgnoreCase);
			JArray users = json["data"]?["users"] as JArray;
			if (users != null)
			{
		...
		{
			JToken tokenData = json["data"]?["streamPlaybackAccessToken"];
			if (tokenData == null || tokenData.Type == JTokenType.Null)
			{
				Logger.Warn("streamPlaybackAccessToken is null for channel: " + channelName);
				return null;
			}
			string token = tokenData["value"]?.ToString();
			string signature = tokenData["signature"]?.ToString();
