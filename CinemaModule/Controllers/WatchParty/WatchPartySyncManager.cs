...
namespace CinemaModule.Controllers.WatchParty
{
	public sealed class WatchPartySyncManager : IDisposable
	{
		...
		private double _pendingSeekTarget = -1.0;

		private DateTime _pendingSeekTime = DateTime.MinValue;

		private double _lastServerTime = -1.0;
		...
		public WatchPartySyncManager(PlaybackController playbackController, Func<MemberState, Task> reportMemberState)
		{
		...
		public bool DetectHostSeek(WatchPartyLocalState state)
		{
			...
			double num = Math.Abs(newServerTime - expectedTime);
			_lastServerTime = newServerTime;
			_lastServerTimeReceived = now;
			return num >= 2.0;
		}
		...
		public void ForceSyncPlayback(WatchPartyLocalState state, bool isLiveStream)
		{
			...
			DebouncedForceSyncAsync(isLiveStream, token);
		}
		...
		public void SyncPlayPauseImmediate(WatchPartyLocalState state)
		{
			...
			if (state.IsPlaying && (isPaused || isBuffering))
			{
				...
				_reportMemberState(MemberState.Playing);
			}
			else if (!state.IsPlaying && (isPlaying || isBuffering))
			{
				_playbackController.Pause();
				_reportMemberState(MemberState.Paused);
			}
		}
		...
		private async Task DebouncedForceSyncAsync(bool isLiveStream, CancellationToken cancellationToken)
		{
			try
			{
				await Task.Delay(150, cancellationToken).ConfigureAwait(continueOnCapturedContext: false);
			}
		...
		private void SyncSeekPosition(WatchPartyLocalState state, bool isLiveStream, bool forceSeek)
		{
			...
			double durationSeconds = (double)durationMs / 1000.0;
			...
			double currentSeconds = (double)_playbackController.Position * durationSeconds;
			double drift = Math.Abs(currentSeconds - targetTime);
			DateTime now = DateTime.UtcNow;
			if (!IsPendingSeekInProgress(currentSeconds, now) && ShouldSeek(forceSeek, drift, now))
			{
		...
		private bool ShouldSeek(bool forceSeek, double drift, DateTime now)
		{
			...
			if (forceSeek)
			{
				return (now - _lastForcedSeekTime).TotalSeconds >= 0.3;
			}
			if (drift <= 3.0)
			{
				return false;
		...
		{
			if ((now - _videoLoadTime).TotalSeconds < 5.0)
			{
				return 3.0;
			}
			if (drift > 10.0)
