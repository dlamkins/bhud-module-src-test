...
namespace FarmingTracker
{
	public class UiUpdater
	{
		public static void UpdateStatPanels(StatsPanels statsPanels, StatsSnapshot snapshot, Model model, Services services)
		{
			...
			(List<Stat> favoriteItems, List<Stat> regularItems) tuple3 = StatsService.SplitIntoFavoriteAndRegularItems(items, favoriteItemApiIds);
			List<Stat> favoriteItems = tuple3.favoriteItems;
			items = tuple3.regularItems;
			...
			(List<Stat> items, List<Stat> currencies) tuple4 = StatsService.RemoveStatsNotUpdatedYetDueToApiError(items, currencies);
			items = tuple4.items;
			currencies = tuple4.currencies;
			...
			(List<Stat> items, List<Stat> currencies) tuple6 = FilterService.FilterStatsAndSetFunnelOpacity(items, currencies, statsPanels, services.SettingService);
			items = tuple6.items;
			...
			ControlCollection<Control> currencyControls = CreateStatControls(currencies, PanelType.SummaryCurrencies, model.IgnoredItemApiIds, model.FavoriteItemApiIds, services);
			ControlCollection<Control> favoriteItemsControls = CreateStatControls(favoriteItems, PanelType.SummaryFavoriteItems, model.IgnoredItemApiIds, model.FavoriteItemApiIds, services);
			ControlCollection<Control> itemControls = CreateStatControls(items, PanelType.SummaryRegularItems, model.IgnoredItemApiIds, model.FavoriteItemApiIds, services);
		...
		private static ControlCollection<Control> CreateStatControls(List<Stat> stats, PanelType panelType, SafeList<int> ignoredItemApiIds, SafeList<int> favoriteItemApiIds, Services services)
		{
			ControlCollection<Control> controls = new ControlCollection<Control>();
			foreach (Stat stat in stats)
			{
				controls.Add((Control)(object)new StatContainer(stat, panelType, ignoredItemApiIds, favoriteItemApiIds, services));
			}
			return controls;
		}
	}
