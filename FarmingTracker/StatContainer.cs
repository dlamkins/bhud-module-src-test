...
namespace FarmingTracker
{
	public class StatContainer : Container
	{
		...
		public StatContainer(Stat stat, PanelType panelType, SafeList<int> ignoredItemApiIds, SafeList<int> favoriteItemApiIds, SafeList<CustomStatProfit> customStatProfits, Services services)
		{
			...
			long? unsigned_customStatProfitInCopper = customStatProfits2.ToListSafe().SingleOrDefault((CustomStatProfit c) => c.BelongsToStat(stat2))?.Unsigned_CustomProfitInCopper;
			AsyncTexture2D statIconTexture = GetStatIconTexture(stat2, services2);
			StatTooltip statTooltip = (_statTooltip = new StatTooltip(stat2, unsigned_customStatProfitInCopper, statIconTexture, panelType, services2));
			...
			((Control)val2).set_Opacity((stat2.Signed_Count > 0) ? 1f : ((float)services2.SettingService.NegativeCountIconOpacitySetting.get_Value() / 255f));
			((Control)val2).set_Size(new Point(iconSize));
			((Control)val2).set_Location(new Point(backgroundMargin + iconMargin));
			((Control)val2).set_Parent((Container)(object)this);
			Label val3 = new Label();
			val3.set_Text(stat2.Signed_Count.ToString());
			...
			val3.set_TextColor((stat2.Signed_Count >= 0) ? services2.SettingService.PositiveCountTextColorSetting.get_Value().GetColor() : services2.SettingService.NegativeCountTextColorSetting.get_Value().GetColor());
