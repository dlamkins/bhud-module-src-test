...
namespace FarmingTracker
{
	public class SetupInstructions
	{
		public static void CreateSetupInstructions(BitmapFont font, FlowPanel addDrfTokenFlowPanel, Services services)
		{
			...
			new FixedWidthHintLabel((Container)(object)addDrfTokenFlowPanel, 480, "Windows 8 or newer because DRF requires websocket technolgy.");
			string registerApiKeyHeader = "Register GW2 API key in BlishHUD";
			AddVerticalSpacing(services, addDrfTokenFlowPanel);
			new HeaderLabel((Container)(object)addDrfTokenFlowPanel, registerApiKeyHeader, font);
			new FixedWidthHintLabel((Container)(object)addDrfTokenFlowPanel, 480, "1. Click on the blish icon at the top of the GW2 window to open the blish settings window (NOT the farming tracker icon).\n2. Click on 'Manage API Keys'.\n3. Follow the instructions there to register an GW2 API key in BlishHUD.\n\nHints:\nMake sure you use the correct sign-in button on the ArenaNet website.\n- For a Steam or Epic GW2 account, you must click the Steam or Epic icon. Do NOT click the 'SIGN IN' button.\n- If your GW2 account is not from Steam or Epic, it is the exact opposite. In that case, you must click the 'SIGN IN' button.\n- Make sure that you grant the GW2 API key all permissions.");
			...
			new FixedWidthHintLabel((Container)(object)addDrfTokenFlowPanel, 480, "1. Click the button below and follow the instructions to setup the drf.dll.\n2. Create a drf account on the website and link it with\nyour GW2 Account(s).\n\nHints:\n- That the drf.dll is not installed correctly is a common issue. Handling dll based addons for GW2 can be complicated.\n- Tipp: setup the drf.dll using 'Raidcore Nexus' or 'arcdps'. They automatically update the drf.dll for you as well. When using arcdps to setup DRF, a common mistake is that the drf.dll is placed in the wrong folder. e.g. the 'Guild Wars 2/addons/arcdps' folder is the wrong place.\n- If possible, avoid using the addon-loader or GW2-Addon-Manager or GW2-UOAOM. They do not support auto updates and make everything more complicated in general.\n- IMPORTANT: The message 'DRF Server Connection: Connected :-)' at the top of this window, means that your DRF token is working (the DRF token will be explained in a later step). This message does NOT mean that you have setup the drf.dll correctly! The drf.dll sends drop data to the DRF server. This module connects to the DRF server to receive those drop data. So this module does not interact with the drf.dll directly. The module does not know whether you installed the drf.dll correctly. So make sure you perform the steps in " + testDrfHeader);
			...
			new FixedWidthHintLabel((Container)(object)addDrfTokenFlowPanel, 480, "1. Click the button below to open the DRF web live tracker.\n2. Use this web live tracker to check if the tracking is working. e.g. by opening an unidentified gear. The items should appear almost instantly in the web live tracker.");
			...
			AddVerticalSpacing(services, addDrfTokenFlowPanel);
			new HeaderLabel((Container)(object)addDrfTokenFlowPanel, "Module shows 'Add GW2 API key in BlishHUD Settings!'", font);
			new FixedWidthHintLabel((Container)(object)addDrfTokenFlowPanel, 480, "First check if you followed the steps in '" + registerApiKeyHeader + "'.\n\nSometimes BlishHUD fails to give a module access to the GW2 API key. That can be caused by a GW2 API timeout when BlishHUD is starting or for other unknown reasons. Possible workarounds:\n- Restart BlishHUD.\n- disable the module, wait a few seconds, then enable the module again.");
			AddVerticalSpacing(services, addDrfTokenFlowPanel);
