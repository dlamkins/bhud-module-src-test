...
namespace System.Threading.Channels
{
	internal static class ChannelUtilities
	{
		...
		internal static ValueTask<T> GetInvalidCompletionValueTask<T>(Exception error)
		{
			...
			{
				OperationCanceledException ex = error as OperationCanceledException;
				obj = ((ex != null) ? Task.FromCanceled<T>(ex.CancellationToken.IsCancellationRequested ? ex.CancellationToken : new CancellationToken(canceled: true)) : Task.FromException<T>(CreateInvalidCompletionException(error)));
			}
			else
			{
				obj = Task.FromException<T>(CreateInvalidCompletionException());
			}
			Task<T> task = (Task<T>)obj;
