...
namespace System.Text.Json
{
	...
	internal ref struct Utf8JsonReader
	{
		...
		private ConsumeNumberResult ConsumeNegativeSign(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i)
		{
			byte b = data[i];
			if (b == 45)
			{
		...
		private ConsumeNumberResult ConsumeZero(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i)
		{
			i++;
			if (i < data.Length)
			{
		...
		private ConsumeNumberResult ConsumeIntegerDigits(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i)
		{
			byte value = 0;
			while (i < data.Length)
			{
		...
		private ConsumeNumberResult ConsumeDecimalDigits(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i)
		{
			if (i >= data.Length)
			{
				if (IsLastSpan)
		...
		private ConsumeNumberResult ConsumeSign(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i)
		{
			if (i >= data.Length)
			{
				if (IsLastSpan)
		...
		private bool SkipAllComments([ScopedRef] ref byte marker)
		{
			while (true)
			{
				if (marker == 47)
		...
		private bool SkipAllComments([ScopedRef] ref byte marker, ExceptionResource resource)
		{
			while (true)
			{
				if (marker == 47)
		...
		private void RollBackState([ScopedRef] in PartialStateForRollback state, bool isError = false)
		{
		...
		private ConsumeNumberResult ConsumeNegativeSignMultiSegment(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i, [ScopedRef] in PartialStateForRollback rollBackState)
		{
			byte b = data[i];
			if (b == 45)
			{
		...
		private ConsumeNumberResult ConsumeZeroMultiSegment(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i, [ScopedRef] in PartialStateForRollback rollBackState)
		{
			i++;
			_bytePositionInLine++;
			byte value;
		...
		private ConsumeNumberResult ConsumeIntegerDigitsMultiSegment(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i)
		{
			byte value = 0;
			int num = 0;
			while (i < data.Length)
		...
		private ConsumeNumberResult ConsumeDecimalDigitsMultiSegment(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i, [ScopedRef] in PartialStateForRollback rollBackState)
		{
			if (i >= data.Length)
			{
				if (IsLastSpan)
		...
		private ConsumeNumberResult ConsumeSignMultiSegment(ref ReadOnlySpan<byte> data, [ScopedRef] ref int i, [ScopedRef] in PartialStateForRollback rollBackState)
		{
			if (i >= data.Length)
			{
				if (IsLastSpan)
		...
		private bool SkipAllCommentsMultiSegment([ScopedRef] ref byte marker)
		{
			while (true)
			{
				if (marker == 47)
		...
		private bool SkipAllCommentsMultiSegment([ScopedRef] ref byte marker, ExceptionResource resource)
		{
			while (true)
			{
				if (marker == 47)
		...
		private int FindLineSeparatorMultiSegment(ReadOnlySpan<byte> localBuffer, [ScopedRef] ref int dangerousLineSeparatorBytesConsumed)
		{
		...
		private void ThrowOnDangerousLineSeparatorMultiSegment(ReadOnlySpan<byte> localBuffer, [ScopedRef] ref int dangerousLineSeparatorBytesConsumed)
		{
		...
		public bool TryGetBytesFromBase64([_003C040126af_002Deea0_002D4cbb_002Db0ec_002D5483efb85291_003ENotNullWhen(true)] out byte[]? value)
		{
