...
namespace Nekres.Musician.Core.Domain
{
	public class RealNote
	{
		...
		{
			return string.Format("{0}{1}", (Octave > Octave.Middle) ? "▲" : ((Octave < Octave.Middle) ? "▼" : string.Empty), Note);
		}

		public override bool Equals(object obj)
		{
			return Equals((RealNote)obj);
		}

		protected bool Equals(RealNote other)
		{
			if (Note == other.Note)
			{
		...
		{
			if (string.IsNullOrEmpty(str) || !Enum.TryParse<Note>(str[0].ToString(), ignoreCase: true, out var key) || !TryParseOctave(str, out var octave))
			{
		...
		{
			octave = Octave.Lowest;
			if (string.IsNullOrEmpty(text) || !Array.Exists(Enum.GetValues(typeof(Note)).Cast<Note>().ToArray(), (Note k) => char.Parse(k.ToString()).Equals(char.ToUpperInvariant(text[0]))))
			{
				return false;
			}
			if (text.Length == 1)
			...
			string octaveMarks = text.Substring(1, text.Length - 1);
