...
namespace Nekres.Regions_Of_Tyria.Controls
{
	internal sealed class MapNotification : Container
	{
		...
		public static void ShowNotification(string header, string footer, Texture2D icon = null, float showDuration = 4f, float fadeInDuration = 2f, float fadeOutDuration = 2f)
		{
			...
			foreach (MapNotification activeScreenNotification in ActiveMapNotifications)
			{
				activeScreenNotification.SlideDown((int)((float)(SmallFont.get_LineHeight() + MediumFont.get_LineHeight()) + 21f));
			}
