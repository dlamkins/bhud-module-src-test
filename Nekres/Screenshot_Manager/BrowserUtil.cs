...
namespace Nekres.Screenshot_Manager
{
	internal class BrowserUtil
	{
		...
		public static async void OpenInDefaultBrowser(string url)
		{
			...
			{
				ScreenshotManagerModule.Logger.Warn("Failed to run '" + exe + "' with arguments '" + argString + "': " + ex.Message);
				Process.Start(url);
				return;
			}
			...
			{
				ForceForegroundWindow(GetProcessWithWindowByName(Path.GetFileNameWithoutExtension(psi.FileName), title)?.MainWindowHandle ?? IntPtr.Zero);
			});
		}
