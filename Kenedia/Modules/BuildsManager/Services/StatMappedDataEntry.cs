	...
	public class StatMappedDataEntry : MappedDataEntry<int, Stat>
	{
		public override async Task<bool> LoadCached(string name, string path, CancellationToken token)
		{
			...
			if (System.IO.File.Exists(path))
			{
				...
				StatMappedDataEntry loaded = JsonConvert.DeserializeObject<StatMappedDataEntry>(json, SerializerSettings.Default);
				if (loaded != null)
				{
					base.Map = loaded.Map;
		...
		public override async Task<bool> Update(string name, ByteIntMap map, string path, Gw2ApiManager gw2ApiManager, CancellationToken token)
		{
			...
			try
			{
				...
				if (missing.Any() && missing.All((object e) => e is int))
				{
					...
					{
						IReadOnlyList<Itemstat> items = await gw2ApiManager.Gw2ApiClient.V2.Itemstats.ManyAsync(ids, token);
						if (token.IsCancellationRequested)
						{
							return false;
						}
						...
						{
							Stat entryItem;
							bool num = base.Items.Values.TryFind((Stat e) => e.Id == item.Id, out entryItem);
							if (entryItem == null)
							{
								entryItem = new Stat
								{
