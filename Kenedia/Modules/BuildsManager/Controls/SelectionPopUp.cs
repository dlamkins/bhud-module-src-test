	...
	public class SelectionPopUp : Control
	{
		...
		public selectionType SelectionType
		{
			...
			set
			{
				...
				{
					((TextInputBase)FilterBox).set_Text(string.Empty);
				}
		...
		private void UpdateLayout()
		{
			...
			if (((TextInputBase)FilterBox).get_Text() != null && ((TextInputBase)FilterBox).get_Text() != string.Empty)
			{
		...
		protected override void Paint(SpriteBatch spriteBatch, Rectangle bounds)
		{
			...
			foreach (SelectionEntry entry in new List<SelectionEntry>(FilteredList))
			{
				...
				else
				{
					...
					SpriteBatchExtensions.DrawStringOnCtrl(spriteBatch, (Control)(object)this, string.Join("; ", strings).Replace(Environment.NewLine, string.Empty), Font, new Rectangle(2 + size + 5, Font.get_LineHeight() + ((Control)FilterBox).get_Height() + i * (size + 5) + Font.get_LineHeight() - 5, size, Font.get_LineHeight()), Color.get_LightGray(), false, (HorizontalAlignment)0, (VerticalAlignment)0);
				}
				i++;
			}
