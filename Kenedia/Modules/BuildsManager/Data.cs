...
namespace Kenedia.Modules.BuildsManager
{
	public class Data : IDisposable
	{
		...
		public void UpdateLanguage()
		{
			...
			string file_path = BuildsManager.s_moduleInstance.Paths.professions + "professions [" + culture + "].json";
		...
		private Texture2D LoadImage(string path, GraphicsDevice graphicsDevice, List<string> filesToDelete, Rectangle region = default(Rectangle), Rectangle default_Bounds = default(Rectangle))
		{
			...
			try
			{
				Texture2D texture = TextureUtil.FromStreamPremultiplied(graphicsDevice, (Stream)new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite));
				double factor = 1.0;
			...
			{
				return BuildsManager.s_moduleInstance.TextureManager.getIcon(Icons.Bug);
			}
		...
		public Data()
		{
			...
			_ = BuildsManager.s_moduleInstance.DirectoriesManager;
			PlaceHolder = BuildsManager.s_moduleInstance.TextureManager.getIcon(Icons.Bug);
			List<string> filesToDelete = new List<string>();
			string culture = BuildsManager.getCultureString();
			_ = BuildsManager.s_moduleInstance.Paths.BasePath + "\\api\\";
			...
			string file_path = BuildsManager.s_moduleInstance.Paths.stats + "stats [" + culture + "].json";
			if (File.Exists(file_path))
			{
			...
			BuildsManager.s_moduleInstance.TextureManager.getIcon(Icons.Bug);
			...
			{
				BuildsManager.s_moduleInstance.DataLoaded = true;
			});
		}
	}
}
