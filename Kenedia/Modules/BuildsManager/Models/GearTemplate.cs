...
namespace Kenedia.Modules.BuildsManager.Models
{
	public class GearTemplate : IDisposable
	{
		...
		public List<Weapon_TemplateItem> Weapons = new List<Weapon_TemplateItem>
		{
			...
			{
				Slot = EquipmentSlots.Weapon1_MainHand
			},
			new Weapon_TemplateItem
			{
				Slot = EquipmentSlots.Weapon1_OffHand
			},
			new Weapon_TemplateItem
			{
				Slot = EquipmentSlots.Weapon2_MainHand
			},
			new Weapon_TemplateItem
			{
				Slot = EquipmentSlots.Weapon2_OffHand
			}
		};
		...
		public List<AquaticWeapon_TemplateItem> AquaticWeapons = new List<AquaticWeapon_TemplateItem>
		{
			...
			{
				Slot = EquipmentSlots.AquaticWeapon1
			},
			new AquaticWeapon_TemplateItem
			{
				Slot = EquipmentSlots.AquaticWeapon2
			}
		};
		...
		public GearTemplate(string code = null)
		{
			...
			{
				ItemStrings[l] = ItemStrings[l].Replace("[", string.Empty).Replace("]", string.Empty);
				int.TryParse(ItemStrings[l], out var id);
				if (id > 0)
				{
					Trinkets[m].Stat = BuildsManager.s_moduleInstance.Data.Stats.Find((API.Stat e) => e.Id == id);
				}
			...
			{
				ItemStrings[k] = ItemStrings[k].Replace("[", string.Empty).Replace("]", string.Empty);
				string[] array = ItemStrings[k].Split('|');
				int.TryParse(array[0], out var stat_id3);
				if (stat_id3 > 0)
				{
					Armor[m].Stat = BuildsManager.s_moduleInstance.Data.Stats.Find((API.Stat e) => e.Id == stat_id3);
				}
				BuildsManager.Logger.Debug("Armor[" + m + "].Stat: " + Armor[m].Stat?.Name);
				int.TryParse(array[1], out var rune_id);
				if (stat_id3 > 0)
				{
					Armor[m].Rune = BuildsManager.s_moduleInstance.Data.Runes.Find((API.RuneItem e) => e.Id == rune_id);
				}
			...
			{
				ItemStrings[j] = ItemStrings[j].Replace("[", string.Empty).Replace("]", string.Empty);
				string[] array2 = ItemStrings[j].Split('|');
				int.TryParse(array2[0], out var stat_id2);
				if (stat_id2 > 0)
				{
					Weapons[m].Stat = BuildsManager.s_moduleInstance.Data.Stats.Find((API.Stat e) => e.Id == stat_id2);
				}
				...
				if (stat_id2 > 0)
				{
					Weapons[m].Sigil = BuildsManager.s_moduleInstance.Data.Sigils.Find((API.SigilItem e) => e.Id == sigil_id);
				}
			...
			for (int i = Trinkets.Count + Armor.Count + Weapons.Count; i < Trinkets.Count + Armor.Count + Weapons.Count + AquaticWeapons.Count; i++)
			{
				...
				if (stat_id > 0)
				{
					AquaticWeapons[m].Stat = BuildsManager.s_moduleInstance.Data.Stats.Find((API.Stat e) => e.Id == stat_id);
				}
				...
				if (sigil1_id > 0)
				{
					AquaticWeapons[m].Sigils[0] = BuildsManager.s_moduleInstance.Data.Sigils.Find((API.SigilItem e) => e.Id == sigil1_id);
				}
				BuildsManager.Logger.Debug("AquaticWeapons[" + m + "].Sigil: " + AquaticWeapons[m].Sigils[0]?.Name);
				int.TryParse(array3[3], out var sigil2_id);
				if (sigil2_id > 0)
				{
					AquaticWeapons[m].Sigils[1] = BuildsManager.s_moduleInstance.Data.Sigils.Find((API.SigilItem e) => e.Id == sigil2_id);
				}
