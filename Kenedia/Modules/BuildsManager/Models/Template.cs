...
namespace Kenedia.Modules.BuildsManager.Models
{
	public class Template : IDisposable
	{
		...
		public Template(string name, string build, string gear)
		{
			...
			Profession = BuildsManager.s_moduleInstance.Data.Professions.Find((API.Profession e) => e.Id == Build?.Profession?.Id);
		...
		public void Save_Unformatted()
		{
			...
			File.WriteAllText(Path, JsonConvert.SerializeObject((object)(from e in BuildsManager.s_moduleInstance.Templates
				where e.Path == Path
				select e into a
		...
		public void Save()
		{
			...
			bool first = true;
			foreach (Template_json template in (from e in BuildsManager.s_moduleInstance.Templates
				where e.Path == Path
				select e into a
				select a.Template_json).ToList())
			{
