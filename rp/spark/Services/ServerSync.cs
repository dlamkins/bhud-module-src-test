...
namespace rp.spark.Services
{
	public class ServerSync : IDisposable
	{
		...
		private static bool PresenceChanged(PlayerPresence previous, PlayerPresence current)
		{
			...
			if (SameTrimmed(previous.AccountName, current.AccountName) && SameTrimmed(previous.OfficialCharacterName, current.OfficialCharacterName) && SameTrimmed(previous.DisplayCharacterName, current.DisplayCharacterName) && SameTrimmed(previous.Race, current.Race) && SameTrimmed(previous.Profession, current.Profession) && SameTrimmed(previous.CustomProfession, current.CustomProfession) && SameTrimmed(previous.ActiveProfileId, current.ActiveProfileId) && previous.IsMature == current.IsMature && SameTrimmed(previous.ActiveProfileName, current.ActiveProfileName) && !(previous.ProfileUpdatedAtTime != current.ProfileUpdatedAtTime) && previous.Status == current.Status && SameTrimmed(previous.Currently, current.Currently) && SameTrimmed(previous.OutOfCharacterInfo, current.OutOfCharacterInfo) && SameTrimmed(previous.LocationName, current.LocationName) && previous.IsLocationHidden == current.IsLocationHidden && previous.Region == current.Region && previous.IsVerified == current.IsVerified && previous.IsInGame == current.IsInGame && previous.HasActiveProfile == current.HasActiveProfile && previous.ShareEnabled == current.ShareEnabled && previous.CanShare == current.CanShare)
			{
				return !SameTrimmed(previous.ShareBlockReason, current.ShareBlockReason);
			}
			return true;
		}
