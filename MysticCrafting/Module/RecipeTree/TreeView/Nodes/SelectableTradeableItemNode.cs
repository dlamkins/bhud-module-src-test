...
{
	public abstract class SelectableTradeableItemNode : TradeableItemNode
	{
		...
		public bool Selected
		{
			...
			private set
			{
				...
				if (Checkbox != null)
				{
					((Control)Checkbox).set_Enabled(!value);
					Checkbox.set_Checked(value);
				}
				foreach (IngredientNode item in ((IEnumerable)((Container)this).get_Children()).OfType<IngredientNode>())
				{
					item.UpdateRelatedNodesAndDescendants();
				}
			}
		}
		...
		protected SelectableTradeableItemNode(Container parent)
			: base(parent)
		{
		}
		...
		public virtual void Build()
		{
			...
			((Control)val).set_Location(new Point(35, 12));
			Checkbox = val;
		}
