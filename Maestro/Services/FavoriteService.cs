...
namespace Maestro.Services
{
	public class FavoriteService
	{
		...
		{
			_favoritesCollection = database.GetCollection<FavoriteEntry>("favorites");
			_favoritesCollection.EnsureIndex((FavoriteEntry x) => x.SongKey, unique: true);
			_favoriteKeys = new HashSet<string>(from f in _favoritesCollection.FindAll()
				select f.SongKey);
		...
		public void ToggleFavorite(Song song)
		{
			...
			{
				_favoritesCollection.DeleteMany((FavoriteEntry x) => x.SongKey == key);
			...
			this.FavoritesChanged?.Invoke(this, EventArgs.Empty);
		}
