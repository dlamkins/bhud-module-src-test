...
namespace Microsoft.AspNetCore.SignalR.Internal
{
	internal class SerializedHubMessage
	{
		...
		public SerializedHubMessage(IReadOnlyList<SerializedMessage> messages)
		{
			...
			{
				SerializedMessage serializedMessage = messages[i];
				SetCacheUnsynchronized(serializedMessage.ProtocolName, serializedMessage.Serialized);
			}
		}
		...
		public ReadOnlyMemory<byte> GetSerializedMessage(IHubProtocol protocol)
		{
			lock (_lock)
			{
				if (!TryGetCachedUnsynchronized(protocol.Name, out var result))
				{
					if (Message == null)
					{
						throw new InvalidOperationException("This message was received from another server that did not have the requested protocol available.");
					}
					result = protocol.GetMessageBytes(Message);
					SetCacheUnsynchronized(protocol.Name, result);
				}
				return result;
			}
		}
		...
		internal IReadOnlyList<SerializedMessage> GetAllSerializations()
		{
			lock (_lock)
			{
				...
				List<SerializedMessage> list = new List<SerializedMessage>(2);
		...
		private void SetCacheUnsynchronized(string protocolName, ReadOnlyMemory<byte> serialized)
		{
			...
			foreach (SerializedMessage cachedItem in _cachedItems)
			{
				if (string.Equals(cachedItem.ProtocolName, protocolName, StringComparison.Ordinal))
				{
					return;
				}
			}
		...
		private bool TryGetCachedUnsynchronized(string protocolName, out ReadOnlyMemory<byte> result)
		{
			...
			if (_cachedItems != null)
			{
				foreach (SerializedMessage cachedItem in _cachedItems)
				{
					if (string.Equals(cachedItem.ProtocolName, protocolName, StringComparison.Ordinal))
					{
						result = cachedItem.Serialized;
						return true;
					}
				}
			}
			result = default(ReadOnlyMemory<byte>);
			return false;
		}
	}
}
