...
namespace Soeed.GuildGeoGuesser.Utils
{
	public static class FlowPanelExtensions
	{
		...
		public static FlowPanel BeginFlowFill(this FlowPanel panel, Container parent)
		{
			//IL_0012: Unknown result type (might be due to invalid IL or missing references)
			panel.set_FlowDirection((ControlFlowDirection)3);
			panel.set_OuterControlPadding(new Vector2(20f, 25f));
		...
		public static FlowPanel AddSetting(this FlowPanel panel, SettingEntry setting, int? widthOverride = null)
		{
			...
			int width = widthOverride ?? ((Control)panel).get_Width();
			if (setting.get_SettingType() == typeof(bool))
			{
				viewContainer.Show(FixedWidthBoolSettingView.FromSetting((SettingEntry<bool>)(object)setting, width));
			}
			else
			{
				viewContainer.Show(SettingView.FromType(setting, width));
			}
			return panel;
		}
		...
		public static FlowPanel AddControl<T>(this FlowPanel container, T control) where T : Control
		{
			((Control)control).set_Parent((Container)(object)container);
			((Container)container).AddChild((Control)(object)control);
			return container;
		}

		public static FlowPanel AddControl<T>(this FlowPanel container, T control, out T generatedControl) where T : Control
		{
			((Control)control).set_Parent((Container)(object)container);
			((Container)container).AddChild((Control)(object)control);
		...
		public static FlowPanel AddString(this FlowPanel panel, string text, out Label generated)
		{
